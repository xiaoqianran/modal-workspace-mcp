...
def make_mcp_server():
    ...
    def sandbox_create(
        ...
        `volumes` maps absolute sandbox mount paths to allowlisted Modal Volume names.
        Modal Secrets and Volumes must be allowlisted by gateway environment variables.
        """
        import modal

        name = validate_sandbox_name(name)
        timeout_seconds = bounded_int(
            timeout_seconds, minimum=60, maximum=86400, field="timeout_seconds"
        )
    ...
    def app_list(environment_name: str | None = None) -> list[dict[str, Any]]:
        """List Modal Apps using the official `modal app list --json` CLI surface."""
